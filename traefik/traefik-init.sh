...
# Adjusts 100.64.0.0/16 (WG subnet) and 192.0.2.20 (wg-easy's IP address in vpn-client-net) if the WG subnet or traefik's IP on vpn-client-net differs
ip route add 100.64.0.0/16 via 192.0.2.20 || true   # || true = don't fail container if route already exists
